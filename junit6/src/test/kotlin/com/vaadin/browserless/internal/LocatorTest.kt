...
internal fun DynaNodeGroup.locatorTest() {
    ...
    beforeEach { MockVaadin.setup() }
    beforeEach { TestingLifecycleHooks.setCurrent(MyLifecycleHook()) }
    afterEach { TestingLifecycleHooks.setCurrent(TestingLifecycleHook.DEFAULT) }
    afterEach { MockVaadin.tearDown() }
    ...
    group("unmocked env") {
        beforeEach { MockVaadin.tearDown(); TestingLifecycleHooks.setCurrent(TestingLifecycleHook.DEFAULT) }
...
fun expectAfterLookupCalled() {
    expect(MyLifecycleHook(true, true)) { TestingLifecycleHooks.getCurrent() }
    TestingLifecycleHooks.setCurrent(MyLifecycleHook())
}
