...
/**
 * A live map of all attributes in this session. Modifications to the map will
 * be reflected to the session and vice versa.
 * <p>
...
public class SessionAttributeMap extends AbstractMap<String, Object> {
    ...
    private static class SessionAttributeEntrySet
        ...
        public int size() {
            int count = 0;
            for (Iterator<String> it = listAttrNames(session).iterator(); it
                    .hasNext();) {
                it.next();
                count++;
            }
        ...
        public boolean add(Entry<String, Object> element) {
            boolean modified = !Objects.equals(
                    session.getAttribute(element.getKey()), element.getValue());
            if (modified) {
        ...
        public boolean contains(Object element) {
            ...
            Entry<?, ?> e = (Entry<?, ?>) element;
            return Objects.equals(session.getAttribute((String) e.getKey()),
                    e.getValue());
        }
    }

    private static class SessionAttributeEntrySetIterator
            implements Iterator<Entry<String, Object>> {
        private final HttpSession session;
        /**
         * Copy the attribute names, otherwise {@link #remove()} would throw
        ...
        public Entry<String, Object> next() {
            ...
            return new SimpleEntry<>(lastAttributeName,
                    session.getAttribute(lastAttributeName));
        }
