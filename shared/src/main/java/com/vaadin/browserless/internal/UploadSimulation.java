...
public final class UploadSimulation {
    ...
    /**
     * The result of a single simulated upload that the handler completed
     * without throwing. Flow validates the accepted mime types and file
     * extensions on the server as well, and rejects the request instead of
     * throwing, so a successful invocation can still have been refused.
     *
     * @param rejected
     *            {@code true} if the handler rejected the file
     * @param rejectionMessage
     *            the rejection message, {@code null} unless {@code rejected}
     */
    public record UploadOutcome(boolean rejected,
            @Nullable String rejectionMessage) {
    }
     ...
     * @return the outcome of the invocation, never {@code null}
     ...
     */
    public static UploadOutcome invokeUpload(UploadHandler handler,
            Element element, VaadinRequest request, String fileName,
        ...
        try {
            ...
            UploadOutcome outcome = new UploadOutcome(event.isRejected(),
                    event.isRejected() ? event.getRejectionMessage() : null);
