...
/**
 * Fires keyboard shortcuts at the current UI.
 * <p>
 * A shortcut registration carries a JavaScript filter expression that the
 * client would evaluate against the key event. There is no client here, so the
 * event data handed to Flow answers that filter directly: it reports a match
 * only for the key and modifiers being simulated, which is what makes exactly
 * the right shortcut listener fire.
 * <p>
...
public final class Shortcuts {
    ...
    /**
     * Take a look at `DomEventListenerWrapper.matchesFilter()` to see why this
     * is necessary. If this stuff stops working, place a breakpoint into the
     * {@code getBoolean}/{@code hasKey} function, to see what kind of keys
     * you're receiving and whether it matches {@code filter}.
     */
    private static final class MockFilterJsonObject extends ObjectNode {
        ...
        static {
            try {
                mgenerateEventKeyFilter = ShortcutRegistration.class
                        .getDeclaredMethod("generateEventKeyFilter", Key.class);
                mgenerateEventModifierFilter = ShortcutRegistration.class
                        .getDeclaredMethod("generateEventModifierFilter",
                                java.util.Collection.class);
                Class<?> hashableKeyClass = ShortcutRegistration.class
            ...
            try {
                ...
                filter = mgenerateEventKeyFilter.invoke(null, key).toString()
                        + " && " + mgenerateEventModifierFilter
                                .invoke(null, hashableModifiers).toString();

                // populate the json object so that KeyDownEvent can be created
                // from it
        ...
        public boolean has(String key) {
            // the "key" is a JavaScript expression which matches the key
            // pressed.
        ...
        private boolean matchesFilter(String key) {
            ...
            if (probeFilter.endsWith(" && (event.stopPropagation() || true)")) {
                probeFilter = probeFilter.substring(0, probeFilter.length()
                        - " && (event.stopPropagation() || true)".length());
            }
            if (probeFilter.endsWith(" && (event.preventDefault() || true)")) {
                probeFilter = probeFilter.substring(0, probeFilter.length()
                        - " && (event.preventDefault() || true)".length());
            }
    ...
    /**
     * Fires a shortcut event with given {@code key} and {@code modifiers} in
    ...
    public static void fireShortcut(Key key, Key... modifiers) {
        ...
        // you won't be able to call those from Kotlin:
        // https://github.com/vaadin/flow/issues/5051
    ...
    public static void _fireShortcut(Component component, Key key,
            Key... modifiers) {
        // all shortcut registration carry a filter with them, in order to
        // filter out
        ...
        // The `matchesFilter()` function is peculiar: it simply checks whether
        // the data
        // contains a boolean value with key 'filter'. We need to fake the json
        // object
        // as if it contained all filters (otherwise `matchesFilter()` would NPE
        // on missing key)
